...
#define BIG_CONSTANT(x) (x##LLU)
	...
	FORCE_INLINE constexpr uint32_t getblock32(const uint32_t* p, int i) { return p[i]; }

	FORCE_INLINE constexpr uint64_t getblock64(const uint64_t* p, int i) { return p[i]; }
	...
	inline uint32_t MurmurHash3_x86_32(const char* key, int len)
	{
